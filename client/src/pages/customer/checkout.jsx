...
export default function Checkout() {
    ...
    const handlePlaceOrder = async () => {
        ...
        try {
            ...
            if (paymentMethod === "cod" || paymentMethod === "later") {
                await clearCart();
                showToast(paymentMethod === "later" ? "Đặt hàng thành công! Chúng tôi sẽ liên hệ xác nhận." : "Đặt hàng thành công!", "success");
                navigate("/my-orders");
            } else if (paymentMethod === "vnpay") {
                const { data: paymentData } = await api.post("/payment/create-payment", { orderId: newOrderId, bankCode: "NCB", language: "vn" });
                if (paymentData.paymentUrl) { await clearCart(); window.location.href = paymentData.paymentUrl; }
                else throw new Error(paymentData.message || "Lỗi tạo cổng thanh toán");
            }
        } catch (error) {
    ...
    const FIELD = "w-full p-3 rounded-xl border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-cyan-500 outline-none";

    return (
        <div className="bg-slate-50 min-h-screen flex flex-col">
            <main className="flex-grow">
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 md:py-12">
                    <h1 className="font-display text-3xl font-bold text-slate-800 mb-8 text-center md:text-left">Thanh toán</h1>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                        <div className="lg:col-span-2 flex flex-col gap-6">
                            <section className="bg-white p-6 rounded-2xl border border-slate-200">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="space-y-1">
                                        ...
                                        <input value={shipInfo.hoTen} onChange={e => setShipInfo({ ...shipInfo, hoTen: e.target.value })} className={FIELD} placeholder="Nhập họ tên" />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-xs font-bold uppercase text-slate-500">Số điện thoại</label>
                                        <input value={shipInfo.sdt} onChange={e => setShipInfo({ ...shipInfo, sdt: e.target.value })} className={FIELD} placeholder="Nhập SĐT" />
                                    </div>
                                    <div className="md:col-span-2 space-y-1">
                                        <label className="text-xs font-bold uppercase text-slate-500">Địa chỉ</label>
                                        <input value={shipInfo.diachi} onChange={e => setShipInfo({ ...shipInfo, diachi: e.target.value })} className={FIELD} placeholder="Địa chỉ nhận hàng" />
                                    </div>
                                    <div className="md:col-span-2 space-y-1">
                                        <label className="text-xs font-bold uppercase text-slate-500">Ghi chú</label>
                                        <textarea value={shipInfo.ghichu} onChange={e => setShipInfo({ ...shipInfo, ghichu: e.target.value })} className={FIELD + " h-20 resize-none"} placeholder="Ghi chú giao hàng (tùy chọn)" />
                                    </div>
                                </div>
                            </section>
                            ...
                            <section className="bg-white p-6 rounded-2xl border border-slate-200">
                                ...
                                <div className="space-y-3">
                                    <label className={`flex items-center gap-4 p-4 rounded-xl border cursor-pointer transition-all ${paymentMethod === "cod" ? "border-cyan-600 bg-cyan-50 ring-1 ring-cyan-600" : "border-slate-200 hover:bg-slate-50"}`}>
                                        <input type="radio" name="payment" value="cod" checked={paymentMethod === "cod"} onChange={() => setPaymentMethod("cod")} className="size-5 accent-cyan-600" />
                                    ...
                                    <label className={`flex items-center gap-4 p-4 rounded-xl border cursor-pointer transition-all ${paymentMethod === "vnpay" ? "border-cyan-600 bg-cyan-50 ring-1 ring-cyan-600" : "border-slate-200 hover:bg-slate-50"}`}>
                                        <input type="radio" name="payment" value="vnpay" checked={paymentMethod === "vnpay"} onChange={() => setPaymentMethod("vnpay")} className="size-5 accent-cyan-600" />
                                        <div className="flex-1">
                                            <div className="flex items-center gap-2">
                                                <span className="block font-bold text-slate-800">Thanh toán qua VNPAY</span>
                        ...
                        <aside className="lg:col-span-1">
                            <div className="sticky top-24 bg-white p-6 rounded-2xl border border-slate-200 border-t-4 border-t-cyan-600">
                                <h3 className="font-bold text-slate-800 text-lg mb-4">Đơn hàng ({cart.length} món)</h3>
                                        ...
                                        <div key={item.idchitietgiohang} className="flex gap-3 py-2 border-b border-slate-50">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex justify-between">
                                                    <p className="text-sm font-bold text-slate-800 truncate pr-2">{item.tenLoaiCa}</p>
                                                    <p className="text-sm font-bold text-cyan-600">{Number(item.thanhTien).toLocaleString()}đ</p>
                                                </div>
                                                <p className="text-xs text-slate-500 mt-0.5">
                                                    {item.tenSize} • x{item.soluong} {item.tenDonViTinh}
                                                    {item.khoiluongDuKien > 0 && <span className="ml-1 text-cyan-600 font-medium">(~{item.khoiluongDuKien.toFixed(1)}kg)</span>}
                                                </p>
                                            </div>
                                        </div>
                                    ))}
                                ...
                                <div className="border-t border-dashed border-slate-300 pt-4 space-y-2">
                                    ...
                                    <div className="flex justify-between text-lg font-bold text-slate-800 pt-2 border-t border-slate-100 mt-2">
                                        <span>Tổng cộng</span>
                                        <span className="text-cyan-600">{total.toLocaleString()}đ</span>
                                    </div>
                                </div>
                                ...
                                <button
                                    ...
                                    disabled={loading}
                                    className={`w-full mt-6 py-3.5 rounded-xl text-white font-bold transition-all flex justify-center items-center gap-2 ${
                                        loading ? "bg-slate-400 cursor-not-allowed" : "bg-cyan-600 hover:bg-cyan-700 active:scale-95"
                                    }`}
                                >
                                    {loading ? (
                                        <>
