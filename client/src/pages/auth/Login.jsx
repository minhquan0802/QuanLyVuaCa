  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (code === 0 && result?.authenticated) {
        const infoResponse = await api.get('/tai-khoan/my-info');
        const infoResult = infoResponse.data.result;

        // 🌟 ĐIỂM TIÊN QUYẾT: Lưu vào React Context trước khi navigate
        setUser(infoResult);

        if (infoResult?.vaitro === 'ADMIN') {
