...
export default function ProductList({ searchTerm }) {
    ...
    const getTotalStock = (fishId) => {
        const fishIdNum = Number(fishId);
        return stockList
            .filter(s => Number(s.idLoaiCa) === fishIdNum)
            .reduce((sum, item) => sum + Number(item.soluongton || 0), 0);
    };
                ...
                filteredList.map((product) => {
                    ...
                    return (
                        <div
                            ...
                            className="group relative flex flex-col bg-white rounded-2xl border border-slate-200 hover:shadow-md hover:-translate-y-1 transition-all duration-300 overflow-hidden cursor-pointer"
                        >
                            <div className="aspect-[4/3] w-full overflow-hidden bg-slate-100 relative">
                                ...
                                ></div>
                                <div className="absolute inset-0 bg-black/0 group-hover:bg-cyan-900/10 transition-colors duration-300"></div>
                            ...
                            <div className="p-5 flex flex-col flex-grow">
                                <div className="flex-grow">
                                    <h3 className="font-display text-lg font-bold text-slate-800 leading-tight mb-2 group-hover:text-cyan-600 transition-colors line-clamp-1">
                                    ...
                                    <div className="mt-2">
                                        {priceInfo ? (
                                            ...
                                            <div className="flex items-center gap-2">
                                                <span className="material-symbols-outlined text-cyan-600 text-sm">call</span>
                                                <span className="text-base font-bold text-cyan-600">Liên hệ báo giá</span>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                <button className="mt-4 w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-cyan-600 text-white font-bold text-sm hover:bg-cyan-700 active:scale-95 transition-all duration-300">
